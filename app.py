...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .run-btn{
    background:var(--accent);color:#080b10;border:none;border-radius:8px;
    font-family:var(--sans);font-weight:600;font-size:14px;padding:12px;
    cursor:pointer;width:100%;margin-top:auto;transition:opacity .15s,transform .1s;
    letter-spacing:.02em;
  }
  ...
  .card .bar{height:3px;border-radius:2px;margin-top:8px;width:0%;transition:width .7s ease;}
  ...
  .list-head{
    display:grid;grid-template-columns:12px 1fr 180px 1fr;gap:14px;
  ...
  .row{
    ...
    align-items:center;animation:fadeIn .3s ease;transition:background .15s;
  }
...
<script>
  ...
  async function run() {
    ...
    if (!email || !email.includes("@")) { setStatus("Enter a valid email.", "err"); return; }
    ...
    document.getElementById("list").innerHTML = `
      <div class="empty">
        ...
        <div>Classifying your emails...</div>
      </div>`;
    ...
    try {
      const res  = await fetch("/run", {
        ...
        body: JSON.stringify({email, password, provider, custom_imap, api_key}),
      });
      const data = await res.json();

      if (!data.success) {
        setStatus(data.error, "err");
        document.getElementById("list").innerHTML = `<div class="empty"><div class="icon">⚠️</div><div>${data.error}</div></div>`;
      } else if (!data.results || data.results.length === 0) {
        setStatus("No unread emails found.", "ok");
        document.getElementById("list").innerHTML = `<div class="empty"><div class="icon">📭</div><div>No unread emails in your inbox.</div></div>`;
        ["green","yellow","red"].forEach(c => { document.getElementById(`cnt-${c}`).textContent = "0"; });
      } else {
        setStatus(`Done — ${data.results.length} email(s) classified.`, "ok");
        render(data.results);
      }
    } catch(e) {
      setStatus("Unexpected error: " + e.message, "err");
    }
    btn.disabled = false;
  }
  ...
  function render(results) {
    ...
    results.forEach(r => counts[r.color] = (counts[r.color]||0) + 1);
    ...
    let html = `<div class="list-head"><span></span><span>Subject</span><span>From</span><span>Reason</span></div>`;
    results.forEach((r, i) => {
      html += `<div class="row" style="animation-delay:${i*35}ms">
